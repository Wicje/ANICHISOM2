...
export function TimeMachine({ isOpen, onClose }: TimeMachineProps) {
  const { snapshots, restoreSnapshot, saveSnapshot } = useOS();
  const [selectedIndex, setSelectedIndex] = useState<number>(0);

  // Real timeline snapshots from OS Context
  const timelineSnapshots = snapshots;
  ...
  const currentSnap = timelineSnapshots[selectedIndex];
    ...
    <div className="fixed inset-0 z-[999] bg-black/95 flex flex-col justify-between p-6 sm:p-10 select-none overflow-hidden font-sans">
      ...
      <div className="flex-1 my-4 flex items-center justify-center relative w-full" style={{ perspective: '1200px' }}>
        {timelineSnapshots.length === 0 ? (
          <div className="flex flex-col items-center gap-4 text-center max-w-md bg-slate-900/80 border border-white/10 p-8 rounded-3xl backdrop-blur-2xl">
            ...
            <div>
              <h3 className="font-bold text-base text-white">No Historical Checkpoints</h3>
              <p className="text-xs text-slate-400 mt-1">
                ContinuaOS Time Machine captures active workspaces, open windows, and virtual filesystem states.
              </p>
            </div>
            <button
              onClick={() => {
                saveSnapshot(`Checkpoint ${new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}`);
                audioSystem.playClick();
              }}
              className="mt-2 px-6 py-2.5 rounded-xl bg-[#10F4A0] text-slate-950 font-bold text-xs hover:bg-[#10F4A0]/90 transition-all shadow-lg shadow-[#10F4A0]/25 flex items-center gap-2"
            >
              <Sparkles className="w-4 h-4" /> Create Checkpoint Now
            </button>
          </div>
        ) : (
          timelineSnapshots.map((snap, idx) => {
            ...
            return (
              ...
              >
                ...
                {/* Real Snapshot Window Thumbnails */}
                <div className="flex-1 py-4 grid grid-cols-2 gap-3">
                  <div className="rounded-2xl bg-white/5 border border-white/10 p-3 flex flex-col justify-between">
                    ...
                    <span className="text-[11px] text-slate-400">Desktop, Documents & Media state preserved</span>
                  </div>
                  <div className="rounded-2xl bg-white/5 border border-white/10 p-3 flex flex-col justify-between">
                    <div className="flex items-center gap-2 text-xs font-semibold text-slate-300">
                    ...
                    <span className="text-[11px] text-slate-400">
                      {snap.windows?.map(w => w.title).slice(0, 3).join(', ') || 'No windows open'}
                    </span>
                  </div>
                </div>
