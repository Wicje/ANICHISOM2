...
export function ProductivitySuite({ window: osWindow }: { window: OSWindow }) {
  ...
  return (
    <div className="w-full h-full flex bg-[var(--os-bg)] text-[var(--os-text)] font-sans shadow-2xl relative overflow-hidden select-none">
      {/* Document Sidebar */}
      <AnimatePresence>
        {sidebarOpen && (
          <motion.div
            ...
            className="shrink-0 bg-[var(--os-surface-dim)] border-r border-[var(--os-border)] flex flex-col overflow-hidden relative z-20"
          >
            <div className="p-3 border-b border-[var(--os-border)] flex items-center justify-between">
              <span className="text-xs font-semibold text-[var(--os-text-muted)] uppercase tracking-wider">Documents</span>
              <button onClick={() => setSidebarOpen(false)} className="p-1 text-[var(--os-text-muted)] hover:text-[var(--os-text)] rounded-lg hover:bg-[var(--os-hover)]"><PanelLeft className="w-3.5 h-3.5" /></button>
            </div>
            <div className="flex-1 overflow-y-auto p-2 custom-scrollbar">
              {docList.map(doc => (
                <div key={doc.id} className={cn(
                  "group/doc flex items-center gap-2 px-2.5 py-1.5 rounded-xl mb-1 cursor-pointer transition-colors border",
                  doc.id === projectId 
                    ? "bg-[var(--os-active)] text-[var(--os-primary)] border-[var(--os-primary)]/30 font-semibold" 
                    : "hover:bg-[var(--os-hover)] text-[var(--os-text-muted)] hover:text-[var(--os-text)] border-transparent"
                )} onClick={() => switchDoc(doc.id)}>
                  <FileText className="w-3.5 h-3.5 shrink-0 text-[var(--os-primary)]" />
                  {renaming === doc.id ? (
                    <input
                      className="flex-1 bg-[var(--os-surface)] border border-[var(--os-primary)] rounded-lg px-1.5 py-0.5 text-xs text-[var(--os-text)] outline-none focus:ring-1 focus:ring-[var(--os-primary)]"
                      value={renameValue}
                  ...
                  <div className="opacity-0 group-hover/doc:opacity-100 flex items-center gap-0.5 transition-opacity shrink-0">
                    <button className="p-1 text-[var(--os-text-muted)] hover:text-[var(--os-primary)] rounded hover:bg-[var(--os-hover)]" title="Rename" onClick={(e) => { e.stopPropagation(); setRenaming(doc.id); setRenameValue(doc.title); }}>
                      <Pencil className="w-3 h-3" />
                    </button>
                    <button className="p-1 text-[var(--os-text-muted)] hover:text-[var(--os-primary)] rounded hover:bg-[var(--os-hover)]" title="Duplicate" onClick={(e) => { e.stopPropagation(); duplicateDoc(doc.id); }}>
                      <Copy className="w-3 h-3" />
                    </button>
                    <button className="p-1 text-[var(--os-text-muted)] hover:text-rose-500 rounded hover:bg-[var(--os-hover)]" title="Delete" onClick={(e) => { e.stopPropagation(); deleteDoc(doc.id); }}>
            ...
            <div className="p-2 border-t border-[var(--os-border)]">
              <button onClick={createNewDoc} className="w-full flex items-center justify-center gap-2 px-3 py-2 text-xs font-bold text-[var(--os-primary)] bg-[var(--os-primary)]/10 hover:bg-[var(--os-primary)]/20 border border-[var(--os-primary)]/30 rounded-xl transition-all shadow-sm">
      ...
      {!sidebarOpen && (
        <button onClick={() => setSidebarOpen(true)} className="absolute left-2 top-2 z-30 p-1.5 text-[var(--os-text-muted)] hover:text-[var(--os-text)] bg-[var(--os-surface)] hover:bg-[var(--os-hover)] rounded-xl border border-[var(--os-border)] shadow-md transition-colors" title="Open document sidebar">
          <PanelLeft className="w-4 h-4" />
        </button>
      )}

      {/* Main Content */}
      <div className="flex-1 flex flex-col min-w-0 relative overflow-hidden bg-[var(--os-bg)]">
      ...
      {/* Ribbon Banner */}
      <div className="relative z-10 flex flex-col bg-[var(--os-surface)] border-b border-[var(--os-border)] shrink-0">
        {/* Document Title & Save Status */}
        <div className="flex items-center justify-between gap-2 px-4 py-1.5 border-b border-[var(--os-border)]">
          <input
            className="text-xs sm:text-sm font-semibold bg-transparent outline-none text-[var(--os-text)] flex-1 min-w-0 placeholder-[var(--os-text-muted)]"
            value={docTitle}
            onChange={(e) => handleDocTitleChange(e.target.value)}
          />
          <span className="text-[10px] font-bold text-[var(--os-primary)] bg-[var(--os-primary)]/10 px-2 py-0.5 rounded-full border border-[var(--os-primary)]/20 uppercase tracking-widest">
            {saveStatus === 'saved' ? 'Synced to FS' : saveStatus === 'saving' ? 'Saving...' : 'Unsaved'}
          </span>
        </div>
        <div className="flex items-center justify-between px-2 pt-2">
          <div className="flex items-center gap-1">
            {(['word', 'sheets', 'slides', 'pdf'] as AppType[]).map((app) => (
               <button
                 ...
                 className={cn(
                   "px-4 py-1.5 text-xs font-semibold rounded-t-xl transition-all border border-transparent border-b-0 flex items-center min-w-[90px] justify-center text-center",
                   activeTab === app 
                     ? "bg-[var(--os-bg)] text-[var(--os-primary)] border-[var(--os-border)] shadow-sm font-bold" 
          ...
          <button 
            ...
            className={cn("px-2.5 py-1 flex flex-col justify-center text-[9px] rounded-t-xl transition-colors uppercase font-bold tracking-wider leading-tight mb-[-1px] border border-transparent border-b-0", workspaceMode === 'agency' ? 'bg-[var(--os-bg)] text-[var(--os-primary)] border-[var(--os-border)]' : 'text-[var(--os-text-muted)] hover:text-[var(--os-text)]')}
        ...
        {/* Toolbar */}
        <div className="flex items-center gap-4 px-4 py-2 bg-[var(--os-bg)] border-t border-[var(--os-border)]">
           <div className="flex items-center gap-1 border-r border-[var(--os-border)] pr-4">
             <button className="p-1.5 text-[var(--os-text-muted)] hover:text-[var(--os-text)] hover:bg-[var(--os-hover)] rounded-lg transition-colors" title="Save Document to Storage & Files" onClick={handleSaveToFS}><Save className="w-4 h-4" /></button>
             <button className="p-1.5 text-[var(--os-text-muted)] hover:text-[var(--os-text)] hover:bg-[var(--os-hover)] rounded-lg transition-colors" title="Print" onClick={() => window.print()}><Printer className="w-4 h-4" /></button>
             <button className="p-1.5 text-[var(--os-text-muted)] hover:text-[var(--os-text)] hover:bg-[var(--os-hover)] rounded-lg transition-colors" title="Share via Self-Host" onClick={() => window.dispatchEvent(new CustomEvent('os:notify', { detail: { title: 'Share Link Created', description: 'Link copied to clipboard.' }}))}><Share2 className="w-4 h-4" /></button>
             <button className="p-1.5 text-[var(--os-text-muted)] hover:text-[var(--os-text)] hover:bg-[var(--os-hover)] rounded-lg transition-colors" title="Export" onClick={handleExport}><Download className="w-4 h-4" /></button>
           </div>
