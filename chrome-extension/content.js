// Inject a meta tag, dataset attribute, or set a global window variable to notify the OS that the extension is active.
try {
  document.documentElement.setAttribute('data-continua-extension', 'active');
  if (document.documentElement.dataset) {
    document.documentElement.dataset.continuaExtension = 'active';
  }
} catch (e) {}

// Post a message so the window listeners catch it safely regardless of CSP
try {
  window.postMessage({ source: 'continua-extension', type: 'continua-extension-ready' }, '*');
} catch (e) {}

// We use a script tag to set the variable in the main page context (if allowed by CSP)
try {
